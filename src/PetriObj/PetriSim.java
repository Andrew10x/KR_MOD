...
public class PetriSim implements Cloneable, Serializable {
    ...
    private final SuitStatistics suitStatistics = new SuitStatistics();
    private StateTime timeState;
   // private static double timeCurr=0;

    private int intervL = 2500;
    private int k = 77;
   ...
   public PetriSim(PetriNet net) {
    ...
    public void doStatistics(double dt) {
        if(getCurrentTime() < 190_000) {
            return;
        } else
        if(!suitStatistics.getStartIsSet()) {//
            int startArrivedN = 0;
            int startDsN = 0;
            int startGdsN = 0;
            int startGundsN = 0;
            ...
            for (PetriP position : listPositionsForStatistica) {
                ...
                //System.out.println(position.getName() + " " + position.getMark());
                if(Objects.equals(position.getName(), "NumberOfArrivedSuits")) {
                   startArrivedN = position.getMark();
                }
                else if(Objects.equals(position.getName(), "NumberOfDamagedSuits")) {
                    startDsN = position.getMark();
                }
                else if(Objects.equals(position.getName(), "GivenAwayDamagedSuits")) {
                    startGdsN = position.getMark();
                }
                else if(Objects.equals(position.getName(), "GivenAwayUndamagedSuits")) {
                    startGundsN = position.getMark();
                }
                else if(Objects.equals(position.getName(), "QueueToOp1")) {
                    skip += position.getMark();
                }
                else if(Objects.equals(position.getName(), "JacketsQueue")) {
                    skip += position.getMark();
                }
                else if(Objects.equals(position.getName(), "JacketsQueue2")) {
                    skip += position.getMark();
                }
                else if(Objects.equals(position.getName(), "DamagedSuits")) {
                    skip2 += position.getMark();
                }
                else if(Objects.equals(position.getName(), "Op1IsFree") ||
                        Objects.equals(position.getName(), "Op2IsFree") ||
                        Objects.equals(position.getName(), "Op4IsFree")) {
                    if(position.getMark() == 0)
                        skip++;
                }
                else if(Objects.equals(position.getName(), "Op5IsFree")) {
                    if(position.getMark() == 0)
                        skip2++;
                }
            }
            /*for (PetriT transition : listT) {
                System.out.println(transition.getName() + " " + transition.getBuffer());
            }*/
            suitStatistics.setStartParams(startArrivedN, startDsN, startGdsN, startGundsN, skip, skip2);
            return;
        }
        if (dt > 0) {
            /*if(Math.floor(getCurrentTime()/intervL) == k) {
                k++;
                suitStatistics.addInMeanUndInIntervals();
            }*/

            for (PetriP position : listPositionsForStatistica) {
                position.changeMean(dt);
                if(Objects.equals(position.getName(), "NumberOfArrivedSuits")) {
                    suitStatistics.ifArrivedNIncreased(position.getMark(), getCurrentTime());
                }
                else if(Objects.equals(position.getName(), "NumberOfDamagedSuits")) {
                    suitStatistics.ifDsNIncreased(position.getMark());
                }
                else if(Objects.equals(position.getName(), "GivenAwayDamagedSuits")) {
                    suitStatistics.ifGdsNIncreased(position.getMark(), getCurrentTime());
                }
                else if(Objects.equals(position.getName(), "GivenAwayUndamagedSuits")) {
                    suitStatistics.ifGundsNIncreased(position.getMark(), getCurrentTime());
                }
            }
        }
        if (dt > 0) {
    ...
    public SuitStatistics getSuitStatistics() {
        return suitStatistics;
    }
    ...
    System.out.print("Buffer in Net  " + this.getName() + "   ");
        for (PetriT transition : listT) {
            System.out.print(transition.getName() + " " + transition.getBuffer() + "  ");
        }
        System.out.println();
    }
